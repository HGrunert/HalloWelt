...
class Chatbot:
#'modelle'/Daten
    def __init__(self,random_antworten=[],ganze_saetze={},bekannte_eingaben=[],passende_antworten=[]): #Erstelle die globalen Variablen und Daten.
        global ganze_saetze #Für direkte genau richtige eingaben.
        ganze_saetze ={} #erstelle ein dictionary
        ganze_saetze['Wie verbinde ich mich mit dem B2?'] = '''Führe den ssh Befehl mit dem -i Parameter aus.
        ...
        passende_antworten.append(libs.lottoziehung.lottoziehung().ausgabe()) #Zahlen sind für jede ausführung jeweils stabil.
        ##Zufällige Antworten[]
        global random_antworten
        random_antworten.extend(['Bitte deutlich schreiben.', 'Wenn du meinst...', 'Bist du da sicher?'])
    ...
    def check_frage(eing,xlist):
        out=-1
        einzelworte = self.zerlege_eingabe(eing.lower())
    ...
    def chat_ai(eing):
        ...
        for check in ganze_saetze.items(): #check the exact answers items gets us a list of tuples of
